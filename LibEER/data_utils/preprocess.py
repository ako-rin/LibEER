...
def feature_extraction(data, sample_rate, extract_bands, time_window, overlap, feature_type):
    ...
    # Map feature type to extractor. For raw signals, we provide two orientations:
    #  - 'raw'    -> (sample, channel, time_points)  i.e., C×T (backward compatible, suitable for CDCN)
    #  - 'raw_tc' -> (sample, time_points, channel)  i.e., T×C (matches many paper notations, e.g., 128×32)
    if feature_type == 'raw_tc':
        fe = raw_extraction_tc
    else:
        fe = {
            'psd': psd_extraction,
            'de': de_extraction,
...
def raw_extraction(data, sample_rate, extract_bands, time_window, overlap, orientation: str = 'ct'):
    """
    Raw EEG data extraction - return 1-second (or time_window-second) segments of time-domain signals directly.

    Inputs
    - data: np.ndarray, shape (channel, time_points)
    - sample_rate: int, e.g., 128 for DEAP, 200 for SEED-V, 128 for DREAMER
    - extract_bands: ignored for raw data
    - time_window: float, seconds per segment (e.g., 1.0)
    - overlap: float, seconds overlapped per step (0 means non-overlap)
    - orientation: 'ct' for (sample, channel, time), 'tc' for (sample, time, channel)

    Returns
    - raw segments as np.ndarray with shape:
        * (num_samples, channel, window_size) if orientation == 'ct' (default, CDCN-friendly)
        * (num_samples, window_size, channel) if orientation == 'tc' (paper notation friendly: 128×32)
    """
    noverlap = int(overlap * sample_rate)
    window_size = int(time_window * sample_rate)

    # Ensure length is an integer multiple of window step to avoid off-by-one issues
    step = window_size - noverlap if noverlap != 0 else window_size
    total_len = data.shape[1]
    usable_len = total_len - ((total_len - window_size) % step)
    if usable_len < window_size:
        # If the trial is shorter than one window (shouldn't happen for DEAP), pad zeros to one window
        pad_len = window_size - total_len
        data = np.pad(data, ((0, 0), (0, pad_len)), mode='constant')
        usable_len = window_size

    # Compute number of segments (strict)
    if noverlap != 0:
        sample_num = (usable_len - window_size) // step + 1
    else:
        sample_num = usable_len // window_size

    # Slice into segments
    if orientation == 'ct':
        raw_data = np.zeros((sample_num, data.shape[0], window_size), dtype=data.dtype)
        t = 0
        for i in range(sample_num):
            raw_data[i, :, :] = data[:, t:t + window_size]
            t += step
        return raw_data
    elif orientation == 'tc':
        raw_data = np.zeros((sample_num, window_size, data.shape[0]), dtype=data.dtype)
        t = 0
        for i in range(sample_num):
            # transpose to time-major per sample
            raw_data[i, :, :] = data[:, t:t + window_size].T
            t += step
        return raw_data
    else:
        raise ValueError("orientation must be 'ct' or 'tc'")


def raw_extraction_tc(data, sample_rate, extract_bands, time_window, overlap):
    """
    Wrapper for raw_extraction returning segments with shape (sample, time, channel),
    aligning with notations like R^{128×32} (time × channels).
    """
    return raw_extraction(data, sample_rate, extract_bands, time_window, overlap, orientation='tc')
