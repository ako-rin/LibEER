...
def merge_to_part(data, label, setting=None):
    ...
    if setting.experiment_mode == "subject-dependent" and setting.cross_trail == 'true':
        ...
        # Flatten by subject across selected sessions; keep trails separated
        # Use enumerated session index (s_idx) to avoid using raw session id 'i' as multiplier
        ...
        # Merge all samples for the same subject across selected sessions into one bucket per subject
        ...
        # Each selected session forms one bucket; use enumerated session index for contiguous mapping
